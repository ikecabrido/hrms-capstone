...
$trainingProgram = !empty($_GET['training_program']) ? $_GET['training_program'] : '________________';
$bondPeriod = !empty($_GET['bond_period']) ? $_GET['bond_period'] : '________________';
$agreementDate = !empty($_GET['agreement_date']) && strtotime($_GET['agreement_date']) !== false ? date('F d, Y', strtotime($_GET['agreement_date'])) : date('F d, Y');

$today = date('F d, Y');
$documentTitle = 'Training Bond Agreement';
...
if ($templateRecord && !empty($templateRecord['template_content'])) {
    ...
    $documentBody = lc_replace_placeholders(lc_get_fallback_template('training_bond'), $employee, $employer);
...
<div class="document-preview">
    ...
    <div class="document-header">
        <h2 class="document-title">TRAINING BOND AGREEMENT</h2>
        <p class="document-subtitle">TRAINING AND BOND UNDERTAKING</p>
    </div>
    ...
    <table class="document-information">
        ...
        <tr>
            <td class="info-label">Training Program</td>
            <td class="info-value"><?= htmlspecialchars($trainingProgram) ?></td>
        </tr>

        <tr>
            <td class="info-label">Bond Period</td>
            <td class="info-value"><?= htmlspecialchars($bondPeriod) ?></td>
        </tr>

        <tr>
            <td class="info-label">Agreement Date</td>
            <td class="info-value"><?= $agreementDate ?></td>
        </tr>
