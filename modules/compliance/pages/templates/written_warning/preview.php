...
$incidentDate = !empty($_GET['incident_date']) && strtotime($_GET['incident_date']) !== false ? date('F d, Y', strtotime($_GET['incident_date'])) : '__________';
$incidentDescription = !empty($_GET['incident_description']) ? $_GET['incident_description'] : '';
$policyViolated = !empty($_GET['policy_violated']) ? htmlspecialchars($_GET['policy_violated']) : '';

$today = date('F d, Y');
$documentTitle = 'Written Warning';
...
if ($templateRecord && !empty($templateRecord['template_content'])) {
    ...
    $documentBody = lc_replace_placeholders(lc_get_fallback_template('written_warning'), $employee, $employer);
...
<div class="document-preview">
    ...
    <div class="document-header">
        <h2 class="document-title">WRITTEN WARNING</h2>
        <p class="document-subtitle">ADMINISTRATIVE WARNING NOTICE</p>
    </div>
    ...
    <table class="document-information">
        ...
        <tr>
            ...
            <td class="info-value"><?= $incidentDate ?></td>
        </tr>

        <tr>
            <td class="info-label">Policy / Rule Violated</td>
            <td class="info-value"><?= htmlspecialchars($policyViolated) ?></td>
        </tr>
    ...
    <div class="document-body">
        <?php if ($incidentDescription !== ''): ?>
            ...
            <p class="pre-wrap"><?= htmlspecialchars($incidentDescription) ?></p>
        <?php endif; ?>
        <?= $documentBody ?>
    </div>
