<?php
$quizEditId = isset($_GET['id']) ? (int) $_GET['id'] : 0;
$isQuizEditMode = $quizEditId > 0;
$quizEditData = null;
$quizModuleId = '';
...
if ($isQuizEditMode) {
    try {
        ...
        $stmt = $pdo->prepare('SELECT * FROM ld_quiz WHERE id = ? LIMIT 1');
        $stmt->execute([$quizEditId]);
        $quiz = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($quiz) {
            $quizEditData = $quiz;
            $quizModuleId = (string) ($quiz['module_id'] ?? '');
            ...
            $questionsStmt->execute(['quiz', $quizEditId]);
...
// Load all modules for the dropdown (server-side)
$quizModules = [];
try {
    require_once dirname(__DIR__, 3) . '/classes/module.php';
    require_once dirname(__DIR__, 5) . '/database/db.php';
    $quizModDb = new Database();
    $quizModPdo = $quizModDb->getConnection();
    ...
    <div class="mode-card">
        ...
        <form id="add-quiz-form" method="post" action="pages/instructor/elearning-subpage/ajax/add-quiz.php">
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:1rem; margin-top:1rem;">
                <label>
                    <span>Module</span>
                    <select id="quiz-module-select" name="module_id" required style="width:100%; margin-top:0.35rem; padding:0.8rem; border-radius:10px; border:1px solid var(--border);">
                        <option value="">Select module</option>
                        <?php foreach ($quizModules as $mod): ?>
                        <option value="<?php echo (int)$mod['id']; ?>"<?php echo ($quizModuleId && (string)$mod['id'] === $quizModuleId) ? ' selected' : ''; ?>><?php echo htmlspecialchars($mod['title']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </label>
                <label>
                    <span>Quiz title</span>
                    <input type="text" name="title" value="<?php echo htmlspecialchars((string) ($quizEditData['title'] ?? '')); ?>" required placeholder="e.g. Final Knowledge Check" style="width:100%; margin-top:0.35rem; padding:0.8rem; border-radius:10px; border:1px solid var(--border);" />
                </label>
                <label>
                    <span>Duration (seconds)</span>
                    <input type="number" name="duration_seconds" min="30" value="600" style="width:100%; margin-top:0.35rem; padding:0.8rem; border-radius:10px; border:1px solid var(--border);" />
                </label>
                <label>
            ...
            <div style="margin-top:2rem; border:1px solid var(--border); border-radius:14px; padding:1rem; background:rgba(255,255,255,0.35);">
                <div style="display:flex; justify-content:space-between; align-items:center; gap:1rem; margin-bottom:1rem; flex-wrap:wrap;">
                    <div>
                        ...
                        <p style="margin:0.35rem 0 0; color:var(--text);">Add quiz questions, choose the question type, and set the correct answer(s).</p>
                    </div>
                    <button type="button" id="add-quiz-question-btn" class="mode-button" style="padding:0.7rem 1rem;">Add Question</button>
                </div>
                <div id="quiz-question-list"></div>
            </div>

            <div class="mode-actions" style="margin-top:1.5rem;">
                <button type="submit" class="mode-button">Save Quiz</button>
...
(function () {
    ...
    const initialQuizData = <?php echo $quizEditData ? json_encode($quizEditData, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) : 'null'; ?>;
    ...
    const addQuestionButton = document.getElementById('add-quiz-question-btn');
    ...
    if (addQuestionButton) {
        addQuestionButton.addEventListener('click', function () {
            ...
            if (!questionList) {
                const fallback = document.getElementById('quiz-question-list');
                if (!fallback) return;
    ...
    form.addEventListener('submit', function (event) {
        ...
        .then(async function (response) {
            ...
            if (!response.ok || !data.success) {
                throw new Error(data.message || (isEditMode ? 'Unable to update quiz.' : 'Unable to create quiz.'));
            }

            if (window.showToast) window.showToast(data.message || (isEditMode ? 'Quiz updated' : 'Quiz created'), 'success');
            if (!isEditMode) {
                form.reset();
        ...
        .catch(function (error) {
            if (window.showToast) window.showToast(error.message || 'Failed to save quiz', 'error');
        })
        .finally(function () {
            if (submitButton) {
    ...
    function applyQuizEditData(quiz) {
        if (!quiz) return;

        const titleInput = form.querySelector('input[name="title"]');
        if (titleInput) titleInput.value = quiz.title || '';
        const durationInput = form.querySelector('input[name="duration_seconds"]');
        if (durationInput) durationInput.value = quiz.duration_seconds || 600;
        const passingScoreInput = form.querySelector('input[name="passing_score"]');
        if (passingScoreInput) passingScoreInput.value = quiz.passing_score || 75;
        const maxAttemptsInput = form.querySelector('input[name="max_attempts"]');
        if (maxAttemptsInput) maxAttemptsInput.value = quiz.max_attempts || 2;
        const questionCountInput = form.querySelector('input[name="question_count"]');
        if (questionCountInput) questionCountInput.value = quiz.question_count || 10;
        const statusSelect = form.querySelector('select[name="status"]');
        if (statusSelect) statusSelect.value = quiz.status || 'active';
        if (moduleSelect) moduleSelect.value = quiz.module_id || '';

        if (form.querySelector('input[name="show_answers_after_submit"]')) {
            form.querySelector('input[name="show_answers_after_submit"]').checked = !!quiz.show_answers_after_submit;
        }
    ...
    if (isEditMode && quizId) {
        if (initialQuizData) {
            applyQuizEditData(initialQuizData);
        } else {
            fetch('pages/instructor/elearning-subpage/ajax/get-quiz-by-id.php?id=' + quizId, {
...
<!-- Module dropdown populated server-side above -->
