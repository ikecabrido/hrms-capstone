...
try {
    ...
    // All video conferences
    $stmt = $pdo->prepare("SELECT vc.id, vc.title, vc.platform, vc.scheduled_at, vc.duration_minutes, CONCAT(emp.first_name,' ',emp.last_name) AS host_name, 'video-conference' AS event_type FROM ld_video_conference vc LEFT JOIN em_employees emp ON emp.employee_id = vc.instructor_id WHERE vc.status = 'scheduled' AND vc.scheduled_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY vc.scheduled_at ASC LIMIT 50");
    $stmt->execute();
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $s) {
        $events[] = ['date'=>date('Y-m-d',strtotime($s['scheduled_at'])),'time'=>date('g:i A',strtotime($s['scheduled_at'])),'title'=>$s['title'],'type'=>'Video Conference','color'=>'#007bff','icon'=>'fa-video','meta'=>$s['platform'].' &bull; '.$s['duration_minutes'].' min','datetime'=>$s['scheduled_at']];
    }

    // Course deadlines
    $stmt = $pdo->prepare("SELECT c.title, c.enrollment_deadline FROM ld_course c WHERE c.status = 'active' AND c.enrollment_deadline IS NOT NULL AND c.enrollment_deadline >= CURDATE() ORDER BY c.enrollment_deadline ASC LIMIT 50");
    $stmt->execute();
    ...
    // Pending reports count by date
    $stmt = $pdo->prepare("SELECT DATE(created_at) AS day, COUNT(*) AS cnt FROM ld_report WHERE status = 'pending' GROUP BY day ORDER BY day DESC LIMIT 10");
    $stmt->execute();
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
        $events[] = ['date'=>$r['day'],'time'=>'','title'=>$r['cnt'].' pending report(s)','type'=>'Reports','color'=>'#fd7e14','icon'=>'fa-flag','meta'=>'Moderation queue','datetime'=>$r['day'].' 09:00:00'];
    }
...
} catch (Throwable $e) { DbError::capture($e, 'admin/calendar'); $events = []; }

// Upcoming events only (nearest to occur first), paginated 10 per page
$upcomingEvents = array_values(array_filter($events, fn($ev) => strtotime($ev['datetime']) >= time()));
$perPage = 12;
$totalEvents = count($upcomingEvents);
$totalPages = max(1, (int) ceil($totalEvents / $perPage));
$epage = max(1, min($totalPages, (int) ($_GET['epage'] ?? 1)));
$pageEvents = array_slice($upcomingEvents, ($epage - 1) * $perPage, $perPage);
...
<div class="module-content">
    <div class="toolbar">
        <div style="display:flex;align-items:center;justify-content:space-between;width:100%;gap:0.75rem;">
            <a href="?page=admin/calendar" style="padding:0.5rem 1rem;background:var(--primary);color:#fff;border:none;border-radius:8px;text-decoration:none;font-size:0.85rem;font-weight:600;white-space:nowrap;">Today</a>
            <div style="display:flex;align-items:center;gap:0.75rem;flex:1;justify-content:center;">
                <a href="?page=admin/calendar&year=<?= $prevYear ?>&month=<?= $prevMonth ?>" style="padding:0.4rem 0.8rem;background:#f0f0f0;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;"><i class="fas fa-chevron-left"></i></a>
                <h2 style="margin:0;white-space:nowrap;font-size:1.2rem;color:var(--text);"> <?= $monthNames[$month] ?> <?= $year ?></h2>
                <a href="?page=admin/calendar&year=<?= $nextYear ?>&month=<?= $nextMonth ?>" style="padding:0.4rem 0.8rem;background:#f0f0f0;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;"><i class="fas fa-chevron-right"></i></a>
            </div>
    ...
    <div class="mode-card" style="margin-top:1.5rem;">
        ...
        <?php else: ?>
            ...
            <?php if ($totalPages > 1): ?>
                <?php
                    $pageLink = fn($p) => '?page=admin/calendar' . ($year != (int) $now->format('Y') || $month != (int) $now->format('m') ? '&year=' . $year . '&month=' . $month : '') . '&epage=' . $p;
                    $startPage = max(1, $epage - 2);
                    $endPage = min($totalPages, $startPage + 4);
                    $startPage = max(1, $endPage - 4);
                ?>
                <div style="display:flex;align-items:center;justify-content:center;gap:0.4rem;margin-top:1.25rem;flex-wrap:wrap;">
                    <?php if ($epage > 1): ?><a href="<?= $pageLink($epage - 1) ?>" style="padding:0.4rem 0.8rem;background:#f0f0f0;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;font-size:0.85rem;"><i class="fas fa-chevron-left"></i></a><?php endif; ?>
                    <?php if ($startPage > 1): ?>
                        <a href="<?= $pageLink(1) ?>" style="padding:0.4rem 0.7rem;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;font-size:0.85rem;<?= 1 === $epage ? 'background:var(--primary);color:#fff;border-color:var(--primary);' : 'background:#f0f0f0;' ?>>1</a>
                        <?php if ($startPage > 2): ?><span style="color:#999;font-size:0.85rem;">&hellip;</span><?php endif; ?>
                    <?php endif; ?>
                    <?php for ($p = $startPage; $p <= $endPage; $p++): ?>
                        <a href="<?= $pageLink($p) ?>" style="padding:0.4rem 0.7rem;border:1px solid #ddd;border-radius:6px;text-decoration:none;font-size:0.85rem;<?= $p === $epage ? 'background:var(--primary);color:#fff;border-color:var(--primary);font-weight:600;' : 'background:#f0f0f0;color:#333;' ?>><?= $p ?></a>
                    <?php endfor; ?>
                    <?php if ($endPage < $totalPages): ?>
                        <?php if ($endPage < $totalPages - 1): ?><span style="color:#999;font-size:0.85rem;">&hellip;</span><?php endif; ?>
                        <a href="<?= $pageLink($totalPages) ?>" style="padding:0.4rem 0.7rem;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;font-size:0.85rem;<?= $totalPages === $epage ? 'background:var(--primary);color:#fff;border-color:var(--primary);' : 'background:#f0f0f0;' ?>><?= $totalPages ?></a>
                    <?php endif; ?>
                    <?php if ($epage < $totalPages): ?><a href="<?= $pageLink($epage + 1) ?>" style="padding:0.4rem 0.8rem;background:#f0f0f0;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;font-size:0.85rem;"><i class="fas fa-chevron-right"></i></a><?php endif; ?>
                </div>
                <p style="text-align:center;color:#999;font-size:0.8rem;margin:0.5rem 0 0;">Showing <?= ($epage - 1) * $perPage + 1 ?>&ndash;<?= min($epage * $perPage, $totalEvents) ?> of <?= $totalEvents ?> upcoming event<?= $totalEvents === 1 ? '' : 's' ?></p>
            <?php endif; ?>
        <?php endif; ?>
    </div>
</div>
...
function openEventModal(date, eventId) {
    ...
    if (eventId) {
        ...
        fetch('pages/admin/ajax/get-calendar-events.php?start=' + date + '&end=' + date)
...
function deleteUserEvent(eventId) {
    openConfirmModal('Are you sure you want to delete this event?', function() {
        ...
        fetch('pages/admin/ajax/delete-calendar-event.php', {method:'POST', body:fd})
        .then(function(r){return r.json()}).then(function(data) {
...
document.getElementById('event-form').addEventListener('submit', function(e) {
    ...
    var url = isEdit ? 'pages/admin/ajax/edit-calendar-event.php' : 'pages/admin/ajax/add-calendar-event.php';
