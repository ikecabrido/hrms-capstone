...
try {
    ...
    // Get upcoming video conference sessions the learner is enrolled in
    $stmt = $pdo->prepare("
        SELECT vc.id, vc.title, vc.platform, vc.scheduled_at, vc.duration_minutes, vc.meeting_link,
               ...
               'video-conference' AS event_type
        FROM ld_video_conference vc
        LEFT JOIN em_employees emp ON emp.employee_id = vc.instructor_id
        LEFT JOIN ld_video_conference_attendee vca ON vca.video_conference_id = vc.id
        WHERE vc.status = 'scheduled' AND vc.scheduled_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)
          AND (vca.learner_id = :learner_id OR vc.instructor_id = :learner_id2)
    ...
    // Get enrollment deadlines (courses not yet completed)
    $stmt = $pdo->prepare("
        SELECT e.course_id, e.enrolled_at, c.title, c.enrollment_deadline, c.start_date,
               'deadline' AS event_type
        FROM ld_enrollment e
        JOIN ld_course c ON c.id = e.course_id
        WHERE e.learner_id = :learner_id AND e.status IN ('enrolled', 'in_progress')
          AND c.enrollment_deadline IS NOT NULL AND c.enrollment_deadline >= CURDATE()
        ORDER BY c.enrollment_deadline ASC
        LIMIT 50
    ");
    ...
    foreach ($upcomingSessions as $s) {
        $events[] = [
            'date' => date('Y-m-d', strtotime($s['scheduled_at'])),
            'time' => date('g:i A', strtotime($s['scheduled_at'])),
            'title' => $s['title'],
            'type' => 'Video Conference',
            'color' => '#007bff',
            'icon' => 'fa-video',
            'meta' => $s['platform'] . ' • ' . $s['duration_minutes'] . ' min • ' . $s['host_name'],
            'datetime' => $s['scheduled_at'],
        ];
    }
    foreach ($upcomingDeadlines as $d) {
        $events[] = [
            'date' => date('Y-m-d', strtotime($d['enrollment_deadline'])),
            'time' => '11:59 PM',
            'title' => $d['title'] . ' — Deadline',
            'type' => 'Deadline',
            'color' => '#dc3545',
            'icon' => 'fa-exclamation-triangle',
            'meta' => 'Enrollment deadline for course',
            'datetime' => $d['enrollment_deadline'],
        ];
    }
...
$firstDayOfWeek = (int) $monthDate->format('w'); // 0=Sun
...
<div class="module-content">
    <div class="toolbar">
        <div style="display:flex;align-items:center;justify-content:space-between;width:100%;gap:0.75rem;">
            <a href="?page=learner/calendar" style="padding:0.5rem 1rem;background:var(--primary);color:#fff;border:none;border-radius:8px;text-decoration:none;font-size:0.85rem;font-weight:600;white-space:nowrap;">Today</a>
            <div style="display:flex;align-items:center;gap:0.75rem;flex:1;justify-content:center;">
                <a href="?page=learner/calendar&year=<?= $prevYear ?>&month=<?= $prevMonth ?>" style="padding:0.4rem 0.8rem;background:#f0f0f0;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;"><i class="fas fa-chevron-left"></i></a>
                <h2 style="margin:0;white-space:nowrap;font-size:1.2rem;color:var(--text);"><?= $monthNames[$month] ?> <?= $year ?></h2>
                <a href="?page=learner/calendar&year=<?= $nextYear ?>&month=<?= $nextMonth ?>" style="padding:0.4rem 0.8rem;background:#f0f0f0;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#333;"><i class="fas fa-chevron-right"></i></a>
            </div>
    ...
    <div class="mode-card" style="padding:0; overflow:hidden;">
        ...
        <div style="display:grid; grid-template-columns:repeat(7, minmax(0,1fr)); ">
            ...
            ?>
                <div style="min-height:100px; padding:0.5rem; background:#fafafa; border:1px solid #f0f0f0; border-radius:0;"></div>
            <?php endfor; ?>
            ...
            ?>
                <div class="cal-day-cell" data-date="<?= $dateStr ?>" data-events='<?= $eventsJson ?>' onclick="openDayModal(this)" style="min-height:100px; padding:0.5rem; border:1px solid #f0f0f0; background:<?= $isToday ? 'rgba(32,0,130,0.05)' : 'var(--surface, #fff)' ?>; position:relative; cursor:pointer; transition:background 0.15s;">
                    ...
                    <?php foreach (array_slice($dayEvents, 0, 3) as $ev): ?>
                        <div style="font-size:0.75rem; padding:0.2rem 0.4rem; margin-bottom:2px; max-width:100%; background:<?= $ev['color'] ?>; color:#fff; border-radius:4px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; pointer-events:none;">
                            <i class="fas <?= $ev['icon'] ?>" style="margin-right:3px;"></i><?= htmlspecialchars($ev['title']) ?>
                        </div>
                    <?php endforeach; ?>
                    <?php if (count($dayEvents) > 3): ?>
                        <div style="font-size:0.7rem; color:#999; padding:0.1rem 0.4rem; pointer-events:none;">+<?= count($dayEvents) - 3 ?> more</div>
                    <?php endif; ?>
                </div>
            <?php endfor; ?>
            ...
            <div style="display:grid; gap:0.75rem; margin-top:1rem;">
                <?php foreach ($events as $ev): ?>
                    <div style="display:flex; align-items:center; gap:1rem; padding:1rem; background:#f9f9f9; border-radius:10px; border-left:4px solid <?= $ev['color'] ?>;">
                        <div style="width:40px; height:40px; border-radius:8px; background:<?= $ev['color'] ?>; color:#fff; display:flex; align-items:center; justify-content:center;">
                            <i class="fas <?= $ev['icon'] ?>"></i>
                        </div>
                        <div style="flex:1;">
                            <strong><?= htmlspecialchars($ev['title']) ?></strong>
                            <p style="margin:0.2rem 0 0 0; color:#666; font-size:0.85rem;">
                                <?= date('l, M j, Y', strtotime($ev['date'])) ?> at <?= $ev['time'] ?>
                                • <?= htmlspecialchars($ev['meta']) ?>
                            </p>
                        </div>
...
<div id="day-modal-overlay" class="modal-overlay" style="display:none; z-index:9999;">
    <div style="background:var(--surface, #fff); border-radius:16px; width:90%; max-width:500px; max-height:80vh; overflow-y:auto; box-shadow:0 20px 60px rgba(0,0,0,0.2);">
        ...
        <div id="day-modal-body" style="padding:1rem 1.5rem 1.5rem;"></div>    </div>
</div>
...
function openDayModal(cell) {
    ...
    if (events.length === 0) {
        body.innerHTML = '<div style="text-align:center; padding:1.5rem; color:rgba(32,0,130,0.4);"><i class="fas fa-calendar-times" style="font-size:2rem; display:block; margin-bottom:0.5rem;"></i><p>No events on this day.</p></div>';
    } else {
        var html = '';
...
document.getElementById('day-modal-overlay').addEventListener('click', function(e) {
    if (e.target.id === 'day-modal-overlay') this.style.display = 'none';
});
...
function openEventModal(date, eventId) {
    ...
    if (eventId) {
        ...
        // Fetch event data
        fetch('pages/learner/ajax/get-calendar-events.php?start=' + date + '&end=' + date)
...
function deleteUserEvent(eventId) {
    openConfirmModal('Are you sure you want to delete this event?', function() {
        ...
        fetch('pages/learner/ajax/delete-calendar-event.php', {method:'POST', body:fd})
        .then(function(r){return r.json()}).then(function(data) {
...
document.getElementById('event-form').addEventListener('submit', function(e) {
    ...
    var url = isEdit ? 'pages/learner/ajax/edit-calendar-event.php' : 'pages/learner/ajax/add-calendar-event.php';
